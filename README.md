# Dossier_Trabajo — Mapa de control FronterIA-Lab / Indioyori

Este repo guarda el corpus de trabajo **y** el dossier maestro para dejar de producir en círculo y **cerrar para cobrar**.

## Empieza aquí

→ **[`dossier/00_LEEME_PRIMERO.md`](dossier/00_LEEME_PRIMERO.md)**

| Documento | Contenido |
|---|---|
| [`dossier/00_LEEME_PRIMERO.md`](dossier/00_LEEME_PRIMERO.md) | Diagnóstico, regla de oro, 2 frentes de la semana |
| [`dossier/01_MAPA_POR_AREAS.md`](dossier/01_MAPA_POR_AREAS.md) | Papers, método, instrumentos, libro, curso, TEKTRON, ingresos — con estado |
| [`dossier/02_INVENTARIO_REPOS.md`](dossier/02_INVENTARIO_REPOS.md) | Qué hay en cada repo + canónicas ante duplicados |
| [`dossier/03_PRIORIZACION_Y_CIERRES.md`](dossier/03_PRIORIZACION_Y_CIERRES.md) | Terminar / vender / congelar + plan 30 días |
| [`dossier/04_GAPS_Y_FALTANTES.md`](dossier/04_GAPS_Y_FALTANTES.md) | Checklist de lo que bloquea cobro |
| [`dossier/05_ESTRATEGIA_SERVICIOS.md`](dossier/05_ESTRATEGIA_SERVICIOS.md) | Menú de 4 SKUs y framing Grieta |
| [`dossier/06_MAPA_CLARO.md`](dossier/06_MAPA_CLARO.md) | Diccionario simple: qué es cada cosa + curso sí/no |
| [`dossier/07_ESTRATEGIA_SOBERANIA.md`](dossier/07_ESTRATEGIA_SOBERANIA.md) | **Estrategia madre:** común gratis vs práctica/intervención de pago |
| [`dossier/08_SITIOS_UNIFICACION.md`](dossier/08_SITIOS_UNIFICACION.md) | Diagnóstico fronteria-lab + Ghost Indioyori + plan unificar/automatizar |
| [`dossier/09_INVESTIGACION_CONSULTORIAS_IA.md`](dossier/09_INVESTIGACION_CONSULTORIAS_IA.md) | Investigación profunda: qué venden las consultoras de IA y tu fit |
| [`dossier/10_PAGINAS_DONDE_CONTRATAN_IA.md`](dossier/10_PAGINAS_DONDE_CONTRATAN_IA.md) | URLs concretas: Workana, Sortlist, CompraNet, Indela, Zinn Hub… |
| [`dossier/herramientas/`](dossier/herramientas/) | One-pagers, SOWs, outline de slides, plan curso MCC |

## Regla operativa

**Máximo 2 frentes activos.** Sin demos → no crear productos. Sin anticipo → no abrir alcance.

## Corpus en la raíz

PDFs y MD de papers, ensayos y análisis político (muchos duplicados también viven en `Marco_Teorico_Indioyori` y `Make-Taller-MCC`). El mapa de arriba indica la **versión canónica** de cada obra.

## Repos hermanos del lab

| Repo | Rol |
|---|---|
| Marco_Teorico_Indioyori | Papers + MACS/IPA + bitácora + blog + keynote |
| Make-Taller-MCC | Curso digital MCC |
| Archivo-final-MCC | Media + libro comercial |
| Libro-indioyori | Drafts del libro (archivo) |
| TEKTRON_Edge | Continuidad infra Edge |
| Laboratorio-de-ingresos | Plan de monetización 2026 |
| hybrid-rag-engine | Stub — congelar |
| industrial-manual-assistant | Stub — congelar |

## Autoría

Dolores Méndez Valdez (Indioyori) — FronterIA-Lab  
Corte del mapa: agosto 2026
